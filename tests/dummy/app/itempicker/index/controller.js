  ...
  _isPublic (copyItem) {
    if (copyItem.access && copyItem.access !== 'public') {
      return {
        ...
        status: {
          status: 'warning',
          i18nKey: 'Warning! This dataset is privately shared. Your applications and data will not be visible to the public'
        }
      };
    } else {
      return {
        ...
        if (err) {
          if (err.code === 499) {
            return {
              ...
              status: {
                status: 'error',
                i18nKey: 'Error! The underlying service is secured. At this time we do not support secured datasets.'
              }
            };
          } else {
            return {
              ...
              status: {
                status: 'error',
                i18nKey: 'Error! Requests to this dataset are timing out. Please check the service'
              }
            };
          }
        }
    ...
    return this._request(httpsUrl)
      .then((response) => {
        ...
        if (err.code === 499) {
          ...
          tempState.status = {
            status: 'error',
            i18nKey: 'Error! This dataset uses HTTP. We only support using HTTPS'
          };
        }
        return tempState;
      });
